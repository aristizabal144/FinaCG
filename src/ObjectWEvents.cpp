...
void ObjectWEvents::setPressColor(ofColor color) {
	hoverColor = color;
}
...
void ObjectWEvents::mousePressed(ofMouseEventArgs& mouse) {
	/*if (inside(mouse.x, mouse.y)) {
		// if the mouse is pressed the object an event will be notified (broadcasted) 
		...
		ofNotifyEvent(pressed, mousePos, this);
		isSelected = true;
		myColor = pressColor;
	}*/
